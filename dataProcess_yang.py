...
def initData4(col_name):
    print(">>>in initData4 function of dataProcess_yang...")
    columns = ['id', 'review', 'service', 'price', 'self', 'outside', 'safety', 'inside', 'dynamic', 'useful', 'times']
    data = pd.read_csv("C:\desktop\Things\Yang\属性分类\\train_multilabel_w0.csv")
    # data = data.loc[:4, columns]

    print("data = ", data.head())

    # y_cols = data.columns.values.tolist()[2:-1]
    # print("y_cols_1 = ", y_cols)

    # data = data.drop(["id", "service", "price", "outside", "times"], axis=1)
    data = data.loc[:, ["review", col_name]]

    y_cols = data.columns.values.tolist()[1:]
    # print("y_cols_2 = ", y_cols)

    data = deleteDataframe(data, col_name)

    value_counts = dict(data[col_name].value_counts())
    print("value_counts_2 = ", value_counts)
...
# 传入一个dataframe，根据两种类别的比例，删除某些行，保证最终比例平衡
def deleteDataframe(data, col_name):

    # 统计不同类别数目
    value_counts = dict(data[col_name].value_counts())
    print("value_counts = ", value_counts)
    print("value_counts' type = ", type(value_counts))
    gap = value_counts.get(0, 0) - value_counts.get(1, 0)
    # test 测试只删除一半
    # gap = gap * 0.7
    print("gap = ", gap)
    delete_index = []  # 生成需要删除的下标
    while gap > 0:
