...
def save_result_to_csv(report, f1_score, experiment_id):
    ...
    with codecs.open("result/result_yang.csv", "a", "utf-8") as f:
        writer = csv.writer(f)
...
def train_cnn_model(model, train, val, train_x, test_x, val_x, epoch,
                    experiment_id, batch_size, learning_rate, y_cols, debug=False, folds=1):
    ...
    for index, col in enumerate(y_cols):
        ...
        # if index >= 1:
        #     break
        experiment_id = col + "_" + experiment_id
